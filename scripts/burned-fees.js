...
const main = async () => {
  ...
  switch (args.interval) {
    case "daily":
      await getDailyData(port, startTimestamp, endTimestamp, args.network, args.network);
      break;
    case "monthly":
...
const fetchData = async (port, query) => {
  ...
  return req.data.data.burnedFees;
};
...
const writeToCsv = (data, interval, network) => {
  ...
  const filePath = `csv-files/${network}-${interval}-burned-fees.csv`;
...
const getDailyData = async (port, start, end, network) => {
  ...
  while (currDay < end) {
    ...
    const burnedFees = await fetchData(port, query);

    for (let burnedFee of burnedFees) {
