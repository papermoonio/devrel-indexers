...
// Current parachain bond account - you mentioned this could change
const PARACHAIN_BOND_ACCOUNT = '0x5a04d242669eDF087Ab5e6829B10D9556F4020a3'.toLowerCase();
const THRESHOLD_AMOUNT = 900000000000000000n; // 10 MOVR
const BLOCK_WHERE_FEE_LOGIC_CHANGED = 10446419;
...
processor.run(new TypeormDatabase(), async (ctx) => {
  ...
  for (const block of ctx.blocks) {
    // Prepare arrays to gather withdraw & deposit events for post–10446419 logic
    ...
    for (const event of block.events) {
      ...
      if (event.name === events.parachainStaking.rewarded.name) {
        ...
        if (block.header.height >= 3202604) {
      ...
      // ---------------------------------
      // 3) PRE-BLOCK-10446419 FEE BURNS
      // ---------------------------------
      else if (block.header.height < 6411588 && event.name === 'Treasury.Deposit') {
        const burnedFee = processTreasuryDeposits(event, block.header.timestamp);
        if (burnedFee !== null) burnedFees.push(burnedFee);
      } else if (
        block.header.height >= 6411588 &&
        ...
        if (events.balances.deposit.v49.is(event)) {
          const [whoFromEvent, balanceFromEvent] = events.balances.deposit.v49.decode(event);
          who = whoFromEvent;
      ...
      // ---------------------------------
      // 4) POST-BLOCK-10446419 FEE BURNS
      // ---------------------------------
        ...
        // (b) Balances.Deposit (possible partial refund)
          ...
          if (events.balances.deposit.v49.is(event)) {
            const [whoFromEvent, balFromEvent] =
              events.balances.deposit.v49.decode(event);
            who = whoFromEvent;
            amount = balFromEvent;
          } else if (events.balances.deposit.v1201.is(event)) {
            const decoded = events.balances.deposit.v1201.decode(event);
...
function processRewards(event: Event, timestamp: number | undefined) {
  ...
  if (events.parachainStaking.rewarded.v49.is(event)) {
    const decodedEvent = events.parachainStaking.rewarded.v49.decode(event);
...
function processParachainBondTransfers(
  ...
  if (events.parachainStaking.reservedForParachainBond.v49.is(event)) {
    const decodedEvent = events.parachainStaking.reservedForParachainBond.v49.decode(event);
    balance = decodedEvent[1];
    // Here we need to extract the destination from the event data if available
    // If not directly available in the event, you might need additional logic or context
    
    // Check if account is provided in the event (this is hypothetical, adapt to your actual event structure)
    if (decodedEvent[0]) {
...
function processTreasuryDeposits(event: Event, timestamp: number | undefined) {
  ...
  // 1) Check if it's a Treasury.Deposit event
  if (events.treasury.deposit.v49.is(event)) {
    balance = events.treasury.deposit.v49.decode(event);
  } else if (events.treasury.deposit.v1300.is(event)) {
    const { value } = events.treasury.deposit.v1300.decode(event);
    balance = value;
  }

  // 2) Or a Balances.Deposit event
  if (events.balances.deposit.v49.is(event)) {
    const [whoFromEvent, balanceFromEvent] = events.balances.deposit.v49.decode(event);
    who = whoFromEvent;
    balance = balanceFromEvent;
