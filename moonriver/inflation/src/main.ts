...
function processRewards(event: Event, timestamp: number | undefined) {
  ...
  if (events.parachainStaking.rewarded.v1300.is(event)) {
...
function processParachainBondTransfers(
  ...
  if (events.parachainStaking.reservedForParachainBond.v1300.is(event)) {
    const decodedEvent =
...
function processTreasuryDeposits(event: Event, timestamp: number | undefined) {
  ...
  if (events.treasury.deposit.v1300.is(event)) {
