...
const fromBlock = 3379186; // BOD Jan 11, 2023
const toBlock = 5919806; // EOD Jan 10, 2023
  ...
  .then((response) => {
    ...
    accounts.forEach((account) => {
      ...
      if (delegatorLefts.length == 0) {
        ...
        saveToCSV([{ accountId: account.id, totalBlocks, currentlyStaking }]);
        return;
      }
...
function saveToCSV(data) {
  const csvWriter = createCsvWriter({
    ...
    header: [
      ...
      { id: 'currentlyStaking', title: 'Currently Staking'}
    ],
    append: true,
  });
