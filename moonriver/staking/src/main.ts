...
function getEventsInfo(ctx: ProcessorContext<Store>): EventsInfo {
  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (event.name == events.parachainStaking.nomination.name) {
        if (events.parachainStaking.nomination.v49.is(event)) {
          let [delegator, _, candidate] =
            events.parachainStaking.nomination.v49.decode(event);
        ...
        } else if (events.parachainStaking.nomination.v53.is(event)) {
          let [delegator, _, candidate] =
            events.parachainStaking.nomination.v53.decode(event);
          ...
          eventsInfo.accountIds.add(delegator);
        } else if (events.parachainStaking.nomination.v900.is(event)) {
          let [delegator, _, candidate] =
            events.parachainStaking.nomination.v900.decode(event);
      ...
      if (event.name == events.parachainStaking.nominatorLeft.name) {
        if (events.parachainStaking.nominatorLeft.v49.is(event)) {
          let [delegator] =
            events.parachainStaking.nominatorLeft.v49.decode(event);
